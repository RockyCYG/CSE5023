...
# 读取loss
with open('results/sentiment-models/epoch_loss_list.txt', 'r') as f:
    loss_list = [float(line.strip()) for line in f if line.strip()]

# 读取accuracy
with open('results/sentiment-models/epoch_acc_list.txt', 'r') as f:
...
plt.title('Sentiment Classification: Loss and Accuracy')
plt.tight_layout(rect=[0, 0, 1, 1])
plt.savefig('sentiment_loss_acc.png', bbox_inches='tight')
plt.show()
